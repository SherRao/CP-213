...
11 English
    ...
    public int compareTo(final Food target) {
    	return target.name == this.name ? Integer.compare(target.calories, this.origin) : target.name.compareTo(this.name);
...
Calories:   calories
    ...
    public String toString() {
    	return new StringJoiner("\n")
    			.add( String.format("Name: %11s", this.name) )
    			.add( String.format("Origin: %11d", this.origin) )
    			.add( String.format("Vegetarian: %11b", this.isVegetarian) )
    			.add( String.format("Calories: %11d", this.calories) )
    			.toString();
    }
