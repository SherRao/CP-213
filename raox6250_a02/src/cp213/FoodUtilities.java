...
public class FoodUtilities {
    ...
    public static Food getFood(final Scanner keyboard) {
    	...
    	boolean isVegetarian = keyboard.nextLine().equals("Y");
    ...
    public static Food readFood(final String line) {
    	...
    	return new Food(input[0], Integer.parseInt(input[1]), Boolean.parseBoolean(input[2]), Integer.parseInt(input[3]));
    ...
    public static ArrayList<Food> foodSearch(final ArrayList<Food> foods, final int origin, final int maxCalories,
    	...
    	ArrayList<Food> list = new ArrayList<>();
    	for(Food food : foods)
    		if( (origin == -1 || food.getOrigin() == origin) && (maxCalories == 0 || food.getCalories() <= maxCalories) && (!isVegetarian || food.isVegetarian()) )
    			list.add(food);
