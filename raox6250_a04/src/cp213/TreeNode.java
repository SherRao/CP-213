...
public class TreeNode<T extends Comparable<T>> {
	...
	private CountedData<T> value = null;
	...
	/**
	 * Creates a new TreeNode with a copy of data and null links to its child
	 * TreeNodes.
	 *
	...
	private final void inOrderAux(final TreeNode<T> node, final ArrayList<CountedData<T>> queue) {
		if (node != null) {
			inOrderAux(node.getLeft(), queue);
			queue.add(new CountedData<T>(node.getValue()));
			inOrderAux(node.getRight(), queue);
